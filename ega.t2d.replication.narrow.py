...
full=0
staticvcf='EGAZ00001231035_v3.clean.all.vcf.gz'
staticphe='egaz.v3.pheno.txt'
id='t2dg13k'

kgp='/data/AnalysisData/Archive/Databases/VCF/AF/ALL.wgs.phase3_shapeit2_mvncall_integrated_v5b.20130502.sites.snpeff.vcf.gz'
exac='/data/AnalysisData/Archive/Databases/VCF/AF/ExAC.r0.3.1.sites.vep.snpeff.vcf.gz'
ensembl='GRCh37.75'
cadd='/data/Software/CADD_v1.3/bin/score.sh'
snpeff='java -jar -Xmx40G /fs/cbsumezey02/storage/Software/snpEff/snpEff.jar'
vcftools='vcftools'
plink='/fs/cbsumezey02/storage/Software/plink'
bgzip='bgzip'
tabix='tabix'
emmax='/fs/cbsumezey02/storage/Software/emmax-beta-07Mar2010/emmax'
emmaxkin='/fs/cbsumezey02/storage/Software/emmax-beta-07Mar2010/emmax-kin'
priority='/fs/cbsumezey02/storage/Software/snpEff/snpeff4.priority.stdin.ann.py'
king='/fs/cbsumezey02/storage/Software/king'
r='R'
...
class SkatSva:
	...
	def skatsva(self,par1,par2,par3,par4,par5,par6,par7,par8,par9,par10,par11,par12):
		...
		if full==0:
			...
			# filter
			print("head -n 1 "+self.csv+"  > f"+self.id+"."+self.filter+".headed.csv")
			print("cat "+self.csv+" | "+self.grep+" |  "+self.awk+" >> f"+self.id+"."+self.filter+".headed.csv")
			print("cat "+self.csv+" | "+self.grep+" |  "+self.awk+" > f"+self.id+"."+self.filter+".csv")
			self.csv="f"+self.id+"."+self.filter+".csv"

			## generate setid
			#print("cat "+self.csv+" | grep -v exac | awk -F\",\" '{print $34,$18}' > f"+self.id+"."+self.filter+".kin.setid")
			print("cat "+self.csv+" | grep -v exac | awk -F\",\" '{print $42,$26}' > f"+self.id+"."+self.filter+".all.setid")

			## filter plink files 
			print("awk '{print $2}' f"+self.id+"."+self.filter+".all.setid > f"+self.id+"."+self.filter+".all.setid.keep")
			print(plink+" --bfile "+self.plinkbedall+" --make-bed --out f"+self.id+"."+self.filter+".all --extract f"+self.id+"."+self.filter+".all.setid.keep")

			# emmax sva with relatives
			print(plink+" --bfile f"+self.id+"."+self.filter+".all --recode 12 transpose --out f"+self.id+"."+self.filter+".all")
			print("cat "+self.phenoall+" | awk -F\"\\"+"t\" '{OFS=\"\\"+"t\";print $1,$2,$6}' | grep -v FID > f"+self.id+"."+self.filter+".all.sva.emmax.pheno")
			print("cat "+self.phenoall+" | awk -F\"\\"+"t\" '{OFS=\"\\"+"t\";print $1,$2,\"1\",$3,$4,$5,$7,$8}' | grep -v FID > f"+self.id+"."+self.filter+".all.sva.emmax.cov")
			print(emmax+" -v -d 10  -t f"+self.id+"."+self.filter+".all -p f"+self.id+"."+self.filter+".all.sva.emmax.pheno -k "+self.kinfall+" -o f"+self.id+"."+self.filter+".all.sva.emmax.age-gender-bmi -c f"+self.id+"."+self.filter+".all.sva.emmax.cov")
			...
			w01.write("o<-read.table(file=\"f"+self.id+"."+self.filter+".all.sva.emmax.age-gender-bmi.ps\",header=F,sep=\"\\t\");\n")
			...
			w01.write("fn<-\"f"+self.id+"."+self.filter+".all.sva.emmax.age-gender-bmi.bonf.OUT.txt\"\n")
			...
			print("./post.processing.py f"+self.id+'.'+self.filter+".all.sva.emmax.age-gender-bmi.bonf.OUT.txt "+self.csv)

			svaallpost="f"+self.id+'.'+self.filter+".all.sva.emmax.age-gender-bmi.bonf.OUT.post.txt"
		...
		if full==0:
			...
			w02.write("q.bed<-\"./f"+self.id+"."+self.filter+".all.bed\";\n")
			w02.write("q.bim<-\"./f"+self.id+"."+self.filter+".all.bim\";\n")
			w02.write("q.fam<-\"./f"+self.id+"."+self.filter+".all.fam\";\n")
			w02.write("q.setid<-\"./f"+self.id+"."+self.filter+".all.setid\";\n")
			w02.write("q.ssd<-\"./f"+self.id+"."+self.filter+".all.ssd\";\n")
			w02.write("q.info<-\"./f"+self.id+"."+self.filter+".all.info\";\n")
			...
			w02.write("fn<-\"f"+self.id+"."+self.filter+".all.skat.emmax.age-gender-bmi.bonf.OUT.txt\";\n")
			...
			print("./post.processing.py f"+self.id+'.'+self.filter+".all.skat.emmax.age-gender-bmi.bonf.OUT.txt "+self.csv)
			skatallpost="f"+self.id+'.'+self.filter+".all.skat.emmax.age-gender-bmi.bonf.OUT.post.TRUE.txt"
			skatallpost1="f"+self.id+'.'+self.filter+".all.skat.emmax.age-gender-bmi.bonf.OUT.post.t2d.TRUE.txt"
			skatallpost2="f"+self.id+'.'+self.filter+".all.skat.emmax.age-gender-bmi.bonf.OUT.post.NOM.txt"
			skatallpost3="f"+self.id+'.'+self.filter+".all.skat.emmax.age-gender-bmi.bonf.OUT.post.t2d.NOM.txt"
		...
		if full==1:
			...
			## generate setid
#			print("cat "+self.csv+" | grep -v exac | awk -F\",\" '{print $34,$18}' > f"+self.id+"."+self.filter+".un.setid")
			print("cat "+self.csv+" | grep -v exac | awk -F\",\" '{print $42,$26}' > f"+self.id+"."+self.filter+".un.setid")
			print("awk '{print $2}' f"+self.id+"."+self.filter+".un.setid > f"+self.id+"."+self.filter+".un.setid.keep")

			## filter plink files 
			print("plink --bfile "+self.plinkbedun+" --make-bed --out f"+self.id+"."+self.filter+".un --extract f"+self.id+"."+self.filter+".un.setid.keep")
		...
		if full==1:
			...
			print(plink+" --bfile f"+self.id+"."+self.filter+".un --recode 12 transpose --out f"+self.id+"."+self.filter+".un")
			print("cat "+self.phenoun+" | awk -F\"\,\" '{OFS=\"\\"+"t\";print $1,$2,$6}' | grep -v fid > f"+self.id+"."+self.filter+".un.sva.emmax.pheno")
			print("cat "+self.phenoun+" | awk -F\"\,\" '{OFS=\"\\"+"t\";print $1,$2,\"1\",$3,$4,$5,$7,$8}' | grep -v id > f"+self.id+"."+self.filter+".un.sva.emmax.cov")
			print(emmax+" -v -d 10  -t f"+self.id+"."+self.filter+".un -p f"+self.id+"."+self.filter+".un.sva.emmax.pheno -k "+self.kinfun+" -o f"+self.id+"."+self.filter+".un.sva.emmax.age-gender-bmi -c f"+self.id+"."+self.filter+".un.sva.emmax.cov")

			# bonf correct emmax sva with relatives

			# sort, significance, and qq plot of emmax with relatives
			w03=open('script.'+self.id+'.'+self.filter+'.un.sva.emmax.age-gender-bmi.bonf.qq.R','w')
			w03.write("o<-read.table(file=\"f"+self.id+"."+self.filter+".un.sva.emmax.age-gender-bmi.ps\",header=F,sep=\"\\t\");\n")
			...
			w03.write("fn<-\"f"+self.id+"."+self.filter+".un.sva.emmax.age-gender-bmi.bonf.OUT.txt\"\n")
			...
			print("./post.processing.py f"+self.id+'.'+self.filter+".un.sva.emmax.age-gender-bmi.bonf.OUT.txt "+self.csv)
			svaunpost="f"+self.id+'.'+self.filter+".un.sva.emmax.age-gender-bmi.bonf.OUT.post.txt"
		...
		if full==1:
			...
			w04.write("q.bed<-\"./f"+self.id+"."+self.filter+".un.bed\";\n")
			w04.write("q.bim<-\"./f"+self.id+"."+self.filter+".un.bim\";\n")
			w04.write("q.fam<-\"./f"+self.id+"."+self.filter+".un.fam\";\n")
			w04.write("q.setid<-\"./f"+self.id+"."+self.filter+".un.setid\";\n")
			w04.write("q.ssd<-\"./f"+self.id+"."+self.filter+".un.ssd\";\n")
			w04.write("q.info<-\"./f"+self.id+"."+self.filter+".un.info\";\n")
			...
			w04.write("fn<-\"f"+self.id+"."+self.filter+".un.skat.null.age-gender-bmi-10pc.bonf.OUT.txt\";\n")
			...
			print("./post.processing.py f"+self.id+'.'+self.filter+".un.skat.null.age-gender-bmi-10pc.bonf.OUT.txt "+self.csv)
			skatunpost="f"+self.id+'.'+self.filter+".un.skat.null.age-gender-bmi-10pc.bonf.OUT.post.TRUE.txt"
			skatunpost1="f"+self.id+'.'+self.filter+".un.skat.null.age-gender-bmi-10pc.bonf.OUT.post.t2d.TRUE.txt"
			skatunpost2="f"+self.id+'.'+self.filter+".all.skat.emmax.age-gender-bmi.bonf.OUT.post.NOM.txt"
			skatunpost3="f"+self.id+'.'+self.filter+".all.skat.emmax.age-gender-bmi.bonf.OUT.post.t2d.NOM.txt"
...
if full==0:
	...
	print("cat "+staticphe+" | grep -v fid | awk '{print $2}' > "+id+".keep")
	# exclude singletons, low quality sites, and sites with high missing rate
	print(vcftools+" --gzvcf "+staticvcf+" --keep "+id+".keep --mac 2 --max-missing .1 --minQ 30 --recode --out "+id+" "+auto)
	print("mv "+id+".recode.vcf "+id+".vcf")
	print("bgzip "+id+".vcf")
	print(tabix+" "+id+".vcf.gz")

if full==0:
	# set aside full dataset

	print("mv "+id+".vcf.gz "+id+".full.vcf.gz")
	print("mv "+id+".vcf.gz.tbi "+id+".full.vcf.gz.tbi")
	
	# limit to genes of interest
	print(vcftools+" --gzvcf "+id+".full.vcf.gz --bed TableI.genes.bed --recode --out "+id)
	print("mv "+id+".recode.vcf "+id+".vcf")
	print("bgzip "+id+".vcf")
	print(tabix+" "+id+".vcf.gz")
...
if full==0:
	### VCF to PLINK
	print(plink+" --vcf "+id+".vcf.gz --make-bed --out "+id+" --const-fid 0")
	# transpose 
	print(plink+" --bfile "+id+" --recode transpose --out "+id+".v2")
	# Update TFAM
	print(" cat "+id+".v2.tfam | ./static.tfam.py "+staticphe+" > "+id+".tfam")
	# add variant ids
	print("cat "+id+".v2.tped | ./update.varid.py > "+id+".v3.tped")
	print(plink+" --tped "+id+".v3.tped --tfam "+id+".tfam --make-bed --out "+id+".v4")

	## maximum missing genotype 10% by individual and by site
	print("plink --bfile "+id+".v4 --make-bed --out "+id+".v5 --geno 0.1 --mind 0.1 ")


if full==0:
	### POPULATION STRUCTURE FOR OUTLIERS
	# pruning
	print(plink+" --bfile "+id+" --indep-pairwise 10000 25 0.25 --make-bed --out "+id+".indep")
	print(plink+" --bfile "+id+".v5 --indep-pairwise 10000 25 0.25 --make-bed --out "+id+".v5.indep")

	## get pca
	print(plink+" --bfile "+id+".indep --pca 10 header tabs --out "+id+".indep")
	print(plink+" --bfile "+id+".v5.indep --pca 10 header tabs --out "+id+".v5.indep")

if full==0:
	# plot PCs
	p01=open('script.'+id+'.v5.indep.pca.R','w')
	p01.write('p<-read.table(file="'+id+'.indep.eigenvec",header=T);')
	...
	p01.write('pdf("'+id+'.indep.pca.pdf");')
	...
	p01.write('dev.off();')
	p01.write('p<-read.table(file="'+id+'.v5.indep.eigenvec",header=T);')
	p01.write('names(p)<-c("fid","iid","pc1","pc2","pc3","pc4","pc5","pc6","pc7","pc8","pc9","pc10");')
	p01.write('x0<-round(summary(p$pc2)[1],3);')
	p01.write('x1<-round(summary(p$pc2)[6],3);')
	p01.write('xs<-(x1-x0)/10;')
	...
	p01.write('dev.new(height=10,width=10);')
	p01.write('par(mar=c(4,7,1,1));')
	p01.write('pdf("'+id+'.v5.indep.pca.pdf");')
	p01.write('plot(p$pc2,p$pc1,frame=F,cex=1.5,pch=21,bg="red",las=1,xlab=NA,xlim=c(x0,x1),xaxp=c(x0,x1,10),ylab=NA,ylim=c(y0,y1),yaxp=c(y0,y1,10));')
	p01.write('mtext(side=1,font=2,line=3,text="PC2");')
	p01.write('mtext(side=2,font=2,line=6,text="PC1");')
	p01.write('axis(side=1,font=2,lwd=2,las=1,at=seq(x0,x1,xs));')
	...
	print(r+' CMD BATCH script.'+id+'.v5.indep.pca.R')
...
if full==0:
	...
	print(plink+" --bfile "+id+".v5 --hardy --out "+id+"")

	# get freq in full cohort
	print(plink+" --bfile "+id+".v5 --freq --out "+id+"")

	## get case and control freq
	print(plink+" --bfile "+id+".v5 --freq case-control --out "+id+"")

	## get ref/alt ac 
	print(vcftools+" --gzvcf "+id+".vcf.gz --counts --out "+id+"")
...
if full==0:
	...
	print(snpeff+" eff "+ensembl+" "+id+".sites.vcf.gz | bgzip -c > "+id+".sites.snpeff.ensembl.vcf.gz")
if full==0:
	# get site list
	print("awk '{print $2}' "+id+".frq | grep -P -v 'SNP' > "+id+".cp")
	print("cat "+id+".cp | perl -pe 's/c//g;s/p/\\t/g' > "+id+".cp.txt")

	# limit annotation to site list
	print(vcftools+" --gzvcf "+id+".sites.snpeff.ensembl.vcf.gz --positions-overlap "+id+".cp.txt --recode --recode-INFO-all --out "+id+".sites.snpeff.ensembl.site-filt")
	print("mv "+id+".sites.snpeff.ensembl.site-filt.recode.vcf "+id+".sites.snpeff.ensembl.site-filt.vcf")
	print(bgzip+" "+id+".sites.snpeff.ensembl.site-filt.vcf")
	print(tabix+" "+id+".sites.snpeff.ensembl.site-filt.vcf.gz")

	# convert annotation to csv format, one annotation per variant
	print("zcat "+id+".sites.snpeff.ensembl.site-filt.vcf.gz | "+priority+" > "+id+".snpeff.ensembl.csv")

	### INTEGRATE
	# frequency and function annotation
	print("cat "+id+".frq | awk '{OFS=\",\"; print $2,$3,$4,$5,$6}' > "+id+".frq.csv")
#	print("paste "+id+".frq.csv "+id+".snpeff.ensembl.csv | perl -pe 's/"+"\\"+"t/,/g' > "+id+".frq.snpeff.ensembl.csv")
	print("cat "+id+".frq.csv | ./integrate.frq-snpeff.py "+id+".snpeff.ensembl.csv > "+id+".frq.snpeff.ensembl.csv")
	print("cat "+id+".frq.snpeff.ensembl.csv | ./integrate.kgp.py kgp."+id+"-sites.INFO | ./integrate.exac.py exac."+id+"-sites.INFO | ./integrate.ac.py "+id+".frq.count | ./integrate.cadd.py "+id.upper()+".cadd13.tsv | ./integrate.ccfreq.py "+id+".frq.cc  | ./integrate.hardy.py "+id+".hwe > "+id+".frq.snpeff.ensembl.kgp.exac.alt.cadd.cc.hwe.csv")
if full==0:
	## GENERAL ANALYSIS
	## plink files  for general analysis, 
	# plink file of all 
	print(plink+" --bfile "+id+".v5 --make-bed --out "+id+".all")

	# transpose for emmax kinship matrix
	print("plink --bfile "+id+".all --missing-genotype 0 --recode 12 transpose --out "+id+".all")
	print(emmaxkin+" -v -d 10 "+id+".all")

	# filter relatives using king
	print("cat "+id+".all.fam  | awk '{print $2,$2,$3,$4,$5,$6}' > "+id+".all.fix.fam")
	print(plink+" --bed "+id+".all.bed --bim "+id+".all.bim --fam "+id+".all.fix.fam --make-bed --out "+id+".all.king-input")
	print(king+" --kinship --ibs -b "+id+".all.king-input.bed --prefix "+id+".all.king-out")
	print(king+" -b "+id+".all.king-input.bed --prefix "+id+".all.king- --unrelated --degree 3")
	print("awk '{print 0,$2}' "+id+".all.king-unrelated.txt > "+id+".unrel.d3.keep")
	print(plink+" --bfile "+id+".all --make-bed --keep "+id+".unrel.d3.keep --out "+id+".unrel")

	# repeat on unrelated 
	# transpose for emmax kinship matrix
	print(plink+" --bfile "+id+".unrel  --missing-genotype 0 --recode 12 transpose --out "+id+".unrel")
	print(emmaxkin+" -v -d 10 "+id+".unrel")

	### POPULATION STRUCTURE
	# pruning
	print(plink+" --bfile "+id+".all --indep-pairwise 10000 25 0.25 --make-bed --out "+id+".all.indep")
	print(plink+" --bfile "+id+".unrel --indep-pairwise 10000 25 0.25 --make-bed --out "+id+".unrel.indep")

	## get pca
	print(plink+" --bfile "+id+".all.indep --pca 10 header tabs --out "+id+".all.indep")
	print(plink+" --bfile "+id+".unrel.indep --pca 10 header tabs --out "+id+".unrel.indep")

	# plot PCs
	p01=open('script.'+id+'.indep.pca.R','w')
	p01.write('p<-read.table(file="'+id+'.all.indep.eigenvec",header=T);')
	p01.write('names(p)<-c("fid","iid","pc1","pc2","pc3","pc4","pc5","pc6","pc7","pc8","pc9","pc10");')
	p01.write('x0<-round(summary(p$pc2)[1],3);')
	p01.write('x1<-round(summary(p$pc2)[6],3);')
	p01.write('xs<-(x1-x0)/10;')
	...
	p01.write('dev.new(height=10,width=10);')
	p01.write('par(mar=c(4,7,1,1));')
	p01.write('pdf("'+id+'.all.indep.pca.pdf");')
	p01.write('plot(p$pc2,p$pc1,frame=F,cex=1.5,pch=21,bg="red",las=1,xlab=NA,xlim=c(x0,x1),xaxp=c(x0,x1,10),ylab=NA,ylim=c(y0,y1),yaxp=c(y0,y1,10));')
	p01.write('mtext(side=1,font=2,line=3,text="PC2");')
	p01.write('mtext(side=2,font=2,line=6,text="PC1");')
	p01.write('axis(side=1,font=2,lwd=2,las=1,at=seq(x0,x1,xs));')
	p01.write('axis(side=2,font=2,lwd=2,las=1,at=seq(y0,y1,ys));')
	p01.write('dev.off();')
	p01.write('p<-read.table(file="'+id+'.unrel.indep.eigenvec",header=T);')
	p01.write('names(p)<-c("fid","iid","pc1","pc2","pc3","pc4","pc5","pc6","pc7","pc8","pc9","pc10");')
	p01.write('x0<-round(summary(p$pc2)[1],3);')
	p01.write('x1<-round(summary(p$pc2)[6],3);')
	p01.write('xs<-(x1-x0)/10;')
	...
	p01.write('dev.new(height=10,width=10);')
	p01.write('par(mar=c(4,7,1,1));')
	p01.write('pdf("'+id+'.unrel.indep.pca.pdf");')
	p01.write('plot(p$pc2,p$pc1,frame=F,cex=1.5,pch=21,bg="red",las=1,xlab=NA,xlim=c(x0,x1),xaxp=c(x0,x1,10),ylab=NA,ylim=c(y0,y1),yaxp=c(y0,y1,10));')
	p01.write('mtext(side=1,font=2,line=3,text="PC2");')
	p01.write('mtext(side=2,font=2,line=6,text="PC1");')
	p01.write('axis(side=1,font=2,lwd=2,las=1,at=seq(x0,x1,xs));')
	...
	print(r+' CMD BATCH script.'+id+'.indep.pca.R')
if full==0:
	## add pca to the phenotype file
	print("./paste.pheno.pca.py "+staticphe+" "+id+".all.indep.eigenvec > "+id+".all.pheno.pca.csv")
	print("cat "+id+".all.pheno.pca.csv | perl -pe 's/,/\\"+"t/g;s/fid\\"+"tiid/FID\\"+"tIID/g;' > "+id+".all.pheno.pca.txt")

	## phenotype file unrelateds only
	print("./unrel.pheno.py "+id+".unrel.d3.keep "+staticphe+" > "+id+".unrel."+staticphe)
	print("./paste.pheno.pca.py "+id+".unrel."+staticphe+" "+id+".unrel.indep.eigenvec > "+id+".unrel.pheno.pca.csv")
	print("cat "+id+".unrel.pheno.pca.csv | perl -pe 's/,/\\"+"t/g;s/fid\\"+"tiid/FID\\"+"tIID/g;' > "+id+".unrel.pheno.pca.txt")
...
plinkbedall=""+id+".all"
kinfall=""+id+".all.BN.kinf"
plinkbedunrel=""+id+".unrel"
kinfun=""+id+".unrel.BN.kinf"
phenoall=""+id+".all.pheno.pca.txt"
phenounrel=""+id+".unrel.pheno.pca.txt"


csv=""+id+".frq.snpeff.ensembl.kgp.exac.alt.cadd.cc.hwe.csv"
